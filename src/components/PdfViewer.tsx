...
// We use the browser's built-in PDF rendering via an iframe with custom controls overlay
// This gives us a reliable cross-browser PDF viewer without heavy dependencies
    ...
    const render = async () => {
      try {
        ...
        if (containerRef.current) {
          containerRef.current.innerHTML = `<div style="padding:40px;text-align:center;color:#94a3b8;">Failed to load PDF. <a href="${url}" target="_blank" style="color:var(--primary-color);">Open in new tab</a></div>`;
        }
      }
    };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="glass rounded-xl p-2 mb-3 flex items-center gap-1 flex-wrap">
        ...
        <span className="text-sm text-slate-600 px-2 min-w-[50px] text-center">{zoom}%</span>
        <button
        ...
        <button
          onClick={() => setZoom(100)}
          className="px-3 py-1.5 rounded-lg hover:bg-white/40 transition text-sm text-slate-600"
          title="Fit to width"
        >
          Fit
        </button>
      ...
      {/* PDF content */}
      <div className="flex-1 overflow-auto glass rounded-xl p-4 bg-white/40">
